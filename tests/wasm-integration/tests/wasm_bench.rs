...
fn performance_comparison() {
    ...
    let vp_r = if hv {
        bench(|| {
            Command::new("vips")
                .args(["thumbnail", &ff, "/dev/null", "32", "--height", "16"])
                .stdout(std::process::Stdio::null())
                .stderr(std::process::Stdio::null())
                .status()
                .unwrap();
    ...
    let ws_r = bench(|| {
        let _ = tr
            .call_resize(&mut st, &wd.pixels, wd.info, 32, 16, ResizeFilter::Lanczos3)
            .unwrap()
            .unwrap();
    });
    ...
    let ws_c = bench(|| {
        let d = bi
            ...
            .unwrap();
        let (rp, ri) = tr
            .call_resize(&mut st, &d.pixels, d.info, 32, 32, ResizeFilter::Lanczos3)
            .unwrap()
            .unwrap();
    ...
    macro_rules! r {
        ($n:expr,$a:expr,$b:expr,$c:expr,$d:expr,$e:expr) => {
            println!(
                "  {:<25} {:>9} {:>9} {:>9} {:>9} {:>9}",
                $n,
                if hm { fmt($a) } else { na.clone() },
                if hv { fmt($b) } else { na.clone() },
                fmt($c),
                fmt($d),
                fmt($e)
            );
    ...
    let hd_file = std::fs::canonicalize(&hd_path)
        .unwrap()
        .to_str()
        .unwrap()
        .to_string();
    ...
    let hd_im_r = if hm {
        bench(|| {
            Command::new("magick")
                .args([&hd_file, "-resize", "960x540!", "null:"])
                .stdout(std::process::Stdio::null())
                .stderr(std::process::Stdio::null())
                .status()
                .unwrap();
    ...
    let hd_vp_r = if hv {
        bench(|| {
            Command::new("vips")
                .args(["thumbnail", &hd_file, "/dev/null", "960", "--height", "540"])
                .stdout(std::process::Stdio::null())
                .stderr(std::process::Stdio::null())
                .status()
                .unwrap();
    ...
    let hd_vp_b = if hv {
        bench(|| {
            Command::new("vips")
                .args(["gaussblur", &hd_file, "/dev/null", "2"])
                .stdout(std::process::Stdio::null())
                .stderr(std::process::Stdio::null())
                .status()
                .unwrap();
    ...
    let hd_ns_b = bench(|| {
        let _ = filters::blur(&hd_dec.pixels, &hd_dec.info, 2.0).unwrap();
    });
    let hd_np_b = bench(|| {
        let mut g = NodeGraph::new(32 << 20);
        let s = g.add_node(Box::new(source::SourceNode::new(hd_data.clone()).unwrap()));
    ...
    let hd_ws_b = bench(|| {
        let _ = fl
            .call_blur(&mut st, &hd_wd.pixels, hd_wd.info, 2.0)
            .unwrap()
            .unwrap();
    });
    ...
    let hd_ws_c = bench(|| {
        let d = bi
            ...
            .unwrap();
        let (rp, ri) = tr
            .call_resize(&mut st, &d.pixels, d.info, 960, 540, ResizeFilter::Lanczos3)
            .unwrap()
            .unwrap();
        let bp = fl.call_blur(&mut st, &rp, ri, 2.0).unwrap().unwrap();
        let sp = fl.call_sharpen(&mut st, &bp, ri, 1.0).unwrap().unwrap();
