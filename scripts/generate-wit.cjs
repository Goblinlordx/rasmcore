...
if (fs.existsSync(PIPELINE_TMPL)) {
  ...
  const content = generatePipelineWit(filters, configStructs);

  // Pipeline imports config records from filters interface
  const usedRecords = new Set();
  for (const f of filters) {
    for (const p of f.params) {
      if (p.type.startsWith('&') && p.type.endsWith('Params')) {
        const structName = p.type.slice(1);
        const info = configStructs.get(structName);
        if (info) {
          usedRecords.add(info.witRecordName);
          // Also import nested records
          for (const field of info.fields) {
            if (configStructs.has(field.type)) {
              usedRecords.add(configStructs.get(field.type).witRecordName);
            }
          }
        }
      }
  ...
  const importLine = usedRecords.size > 0
    ? `    use filters.{${[...usedRecords].sort().join(', ')}};`
    : '';

  let output = tmpl
    .replace('{{GENERATED_PIPELINE_FILTERS}}', content)
    .replace('{{GENERATED_PIPELINE_IMPORTS}}', importLine);
  fs.writeFileSync(PIPELINE_WIT, output);
  console.log(`Generated: wit/image/pipeline.wit (${filters.length} pipeline methods, ${usedRecords.size} imported records)`);
}
