// Liquify twirl — rotate pixels around brush center
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  // Inverse twirl: rotate by -angle*w
  let theta = -params.angle_rad * w;
  let cos_t = cos(theta);
  let sin_t = sin(theta);
  let sx = params.center_x + ddx * cos_t - ddy * sin_t;
  let sy = params.center_y + ddx * sin_t + ddy * cos_t;
