// Vibrance — perceptual saturation boost via HSL, f32 per-pixel
// amount = -100..100 (normalized to -1..1 by Rust side)

struct Params {
  width: u32,
  height: u32,
  amount_norm: f32,  // amount / 100.0
  _pad: u32,
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  let sat = select((mx - mn) / mx, 0.0, mx <= 0.0);
  let scale = params.amount_norm * (1.0 - sat);

  var hsl = rgb_to_hsl(px.r, px.g, px.b);
  hsl.y = clamp(hsl.y * (1.0 + scale), 0.0, 1.0);
