// Box blur f32 — separable H+V averaging filter
...
fn blur_h(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  for (var i = -r; i <= r; i++) {
    ...
    sum += unpack_f32(input[u32(sx) + y * params.width]);
  }
  output[x + y * params.width] = pack_f32(sum / diameter);
}
...
fn blur_v(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  for (var i = -r; i <= r; i++) {
    ...
    sum += unpack_f32(input[x + u32(sy) * params.width]);
  }
  output[x + y * params.width] = pack_f32(sum / diameter);
}
