// Liquify pinch — contract pixels toward brush center
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  // Pinch inverse: source = center + (output - center) / (1 - w)
  let s = 1.0 / max(1.0 - w, 0.01);
  let sx = params.center_x + ddx * s;
  let sy = params.center_y + ddy * s;
