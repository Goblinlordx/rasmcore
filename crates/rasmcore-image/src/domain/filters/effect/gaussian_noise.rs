...
#[filter(name = "gaussian_noise", category = "effect", group = "noise", variant = "gaussian", reference = "additive Gaussian noise")]
...
impl CpuFilter for GaussianNoiseParams {
    fn compute(
        &self,
        request: Rect,
    ...
    let mut rng = self.seed.max(1); // avoid zero state
