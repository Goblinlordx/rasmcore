...
/// Encode function signature for registry dispatch.
pub type EncodeFn = fn(&[u8], &ImageInfo) -> Result<Vec<u8>, ImageError>;

/// Preferred output color space for an encoder format.
///
/// Determines which color space the pipeline should convert to before encoding.
/// The pipeline auto-inserts an output transform (OT) node when the working
/// space doesn't match the format's preferred color space.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum EncoderColorSpace {
    /// sRGB with gamma encoding (PNG, JPEG, WebP, BMP, GIF, etc.)
    Srgb,
    /// Linear (scene-referred) — no gamma (EXR, HDR, FITS)
    Linear,
}

/// Static encoder registration — generated by `#[register_encoder]`.
#[derive(Debug)]
pub struct StaticEncoderRegistration {
    ...
    /// Encode function — dispatches to the per-format encoder with default config.
    /// Optional for backward compatibility; encoders should provide this.
    pub encode_fn: Option<EncodeFn>,
    /// Preferred output color space for this format.
    /// Pipeline auto-inserts OT to convert working space → this before encoding.
    /// Default: sRGB (most common for web/display formats).
    pub preferred_output_cs: EncoderColorSpace,
}
...
/// Look up the preferred output color space for a format.
///
/// Returns the encoder's declared preferred CS, or `Srgb` as fallback
/// if the format is not registered.
pub fn output_color_space_for_format(format: &str) -> EncoderColorSpace {
    for reg in inventory::iter::<&StaticEncoderRegistration> {
        if reg.format == format || reg.extensions.contains(&format) {
            return reg.preferred_output_cs;
        }
    }
...
pub fn encode(
    ...
    // Dispatch to registered per-format encoders via inventory
    for reg in inventory::iter::<&StaticEncoderRegistration> {
        if reg.format == format || reg.extensions.contains(&format) {
            if let Some(f) = reg.encode_fn {
                return f(pixels, info);
            }
        }
    }
