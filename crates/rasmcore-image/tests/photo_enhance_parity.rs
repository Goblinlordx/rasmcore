...
"#,
    ...
    let reference = match output {
        ...
        Ok(o) => {
            eprintln!(
                "SKIP dehaze_vs_python: Python reference script failed: {}",
...
fn dehaze_clear_image_near_identity() {
    ...
    let result = filters::DehazeParams {
        patch_radius: 7,
        omega: 0.95,
        t_min: 0.1,
    }
...
fn clarity_vs_python_usm_midtone() {
    ...
    // Our clarity
    let ours = filters::ClarityParams { amount, sigma }
        .compute(
            Rect::new(0, 0, info.width, info.height),
...
fn clarity_zero_amount_is_identity() {
    ...
    let result = filters::ClarityParams {
        amount: 0.0,
        sigma: 10.0,
    }
    .compute(
...
fn pyramid_detail_remap_large_sigma_near_identity() {
    ...
    let result = filters::PyramidDetailRemapParams {
        sigma: 100.0,
        num_levels: 4,
    }
    .compute(
...
fn pyramid_detail_remap_vs_python_pyramid() {
    ...
    // Our Local Laplacian
    let ours = filters::PyramidDetailRemapParams {
        sigma,
        num_levels: levels,
    }
    .compute(
...
fn ssr_vs_python_opencv_blur() {
    ...
    let ours = filters::RetinexSsrParams { sigma }
