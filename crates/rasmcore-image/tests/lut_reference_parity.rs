...
fn cube_lut_parity_vs_ffmpeg() {
    ...
    if !output.status.success() {
        eprintln!("  ffmpeg lut3d failed: {}", String::from_utf8_lossy(&output.stderr));
        let _ = std::fs::remove_dir_all(&tmp);
        return;
    }
    ...
    let ffmpeg_rgb = if ffmpeg_decoded.info.format == PixelFormat::Rgba8 {
        ffmpeg_decoded.pixels.chunks_exact(4)
    ...
    assert_eq!(our_output.len(), ffmpeg_rgb.len(),
        "output size mismatch: ours={} ffmpeg={}", our_output.len(), ffmpeg_rgb.len());
...
fn hald_lut_parity_vs_imagemagick() {
    ...
    if !out.status.success() {
        eprintln!("  magick hald: failed: {}", String::from_utf8_lossy(&out.stderr));
        let _ = std::fs::remove_dir_all(&tmp);
        return;
    }
    ...
    let out = Command::new("magick")
        ...
        .output().unwrap();
    if !out.status.success() {
        eprintln!("  magick modulate failed: {}", String::from_utf8_lossy(&out.stderr));
        let _ = std::fs::remove_dir_all(&tmp);
        return;
    }
    ...
    let out = Command::new("magick")
        ...
        .output().unwrap();
    if !out.status.success() {
        eprintln!("  magick -hald-clut failed: {}", String::from_utf8_lossy(&out.stderr));
        let _ = std::fs::remove_dir_all(&tmp);
        return;
    }
    ...
    let hald_rgb = if hald_decoded.info.format == PixelFormat::Rgba8 {
        let rgb: Vec<u8> = hald_decoded.pixels.chunks_exact(4)
            .flat_map(|c| [c[0], c[1], c[2]])
            .collect();
    ...
    let our_lut = rasmcore_image::domain::color_lut::parse_hald_lut(
        &hald_rgb.0, &hald_rgb.1
    ).unwrap();
    ...
    assert_eq!(our_output.len(), im_rgb.len(),
        "output size mismatch: ours={} IM={}", our_output.len(), im_rgb.len());
...
fn cube_identity_parity_vs_ffmpeg() {
    ...
    eprintln!("  Identity .cube: us_vs_ffmpeg={mae_us_vs_ff:.4}, us_vs_orig={mae_us_vs_orig:.4}, ff_vs_orig={mae_ff_vs_orig:.4}");
    assert!(mae_us_vs_ff < 1.0, "identity LUT: our vs ffmpeg should be near-zero, got {mae_us_vs_ff}");
    assert!(mae_us_vs_orig < 1.0, "identity LUT: our output should match input, got {mae_us_vs_orig}");
