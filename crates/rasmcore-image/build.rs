...
fn main() {
    ...
    // Replaces deprecated generate-wit.cjs — build.rs is the single source of truth.
    {
        ...
        // NOTE: filters.wit is generated by scripts/generate-wit.cjs (run manually when filters change).
        // build.rs only generates pipeline.wit (which changes with transforms/graph features).
        ...
        if tmpl_path.exists() {
            ...
            // Fill template (filter imports + transform types are in the template)
