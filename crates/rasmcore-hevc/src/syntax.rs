...
pub fn parse_slice_header(
    ...
    // HEVC Section 7.3.6.1: deblocking_filter_override_flag is present only when
    // BOTH deblocking_filter_control_present_flag AND deblocking_filter_override_enabled_flag
    // are true. When override_enabled is false, the flag is inferred to be 0.
    if pps.deblocking_filter_control_present {
        let deblocking_filter_override = if pps.deblocking_filter_override_enabled {
            r.read_flag()?
        } else {
            false // inferred per spec
        };
