...
def golden_otsu_threshold() -> dict:
    ...
    thresh_val, binary_u8 = cv2.threshold(gray_u8, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    binary = binary_u8.astype(np.float32) / 255.0
    output = np.stack([binary, binary, binary], axis=-1)
    return {
        "filter": "otsu_threshold", "params": {},
        "tool": f"cv2.threshold (THRESH_OTSU, threshold={thresh_val})",
        "tool_version": cv2.__version__,
        "note": f"OpenCV Otsu on BT.709 luma u8, threshold={thresh_val}",
        "output": pixels_to_list(output),
    }
...
def golden_canny(low: float, high: float) -> dict:
    ...
    return {
        ...
        "tool_version": cv2.__version__,
        "note": "OpenCV Canny (Gaussian + Sobel + NMS + hysteresis) on BT.709 luma u8",
        "output": pixels_to_list(output),
    }
...
def golden_triangle_threshold() -> dict:
    """Triangle threshold via cv2.threshold (OpenCV C++ implementation)."""
    img = SPATIAL_INPUT_LINEAR.astype(np.float32)
    gray = img[:, :, 0] * 0.2126 + img[:, :, 1] * 0.7152 + img[:, :, 2] * 0.0722
    gray_u8 = np.clip(gray * 255 + 0.5, 0, 255).astype(np.uint8)
    thresh_val, binary_u8 = cv2.threshold(gray_u8, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_TRIANGLE)
    binary = binary_u8.astype(np.float32) / 255.0
    output = np.stack([binary, binary, binary], axis=-1)
    return {
        "filter": "triangle_threshold", "params": {},
        "tool": f"cv2.threshold (THRESH_TRIANGLE, threshold={thresh_val})",
        "tool_version": cv2.__version__,
        "note": f"OpenCV triangle threshold on BT.709 luma u8, threshold={thresh_val}",
        "output": pixels_to_list(output),
    }
