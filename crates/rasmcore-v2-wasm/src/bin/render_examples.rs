...
fn showcase_params(name: &str, params: &[v2::ParamDescriptor]) -> ParamMap {
    ...
    for p in params {
        ...
        match p.value_type {
            ...
            v2::ParamType::U32 | v2::ParamType::I32 => {
    ...
    for name in ["landscape", "portrait", "architecture"] {
        ...
        if path.exists() {
            if let Ok(d) = rasmcore_codecs_v2::decode(&fs::read(&path).unwrap()) {
                fs::write(
                    out.join(format!("reference-{}.png", name)),
                    encode_png(&d.pixels, d.info.width, d.info.height),
                )
                .unwrap();
            }
        }
    ...
    for f in &filters {
        ...
        match g.request_full(fid) {
            Ok(out_px) => {
                ...
                fs::write(
                    out.join(format!("{}-after.png", f.name)),
                    encode_png(&out_px, out_info.width, out_info.height),
                )
                .unwrap();
                ok += 1;
            }
