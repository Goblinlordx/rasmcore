...
fn auto_level_gpu_returns_3_passes() {
    ...
    assert_eq!(
        shaders.len(),
        3,
        "AutoLevel should use 3-pass ChannelMinMax reduction"
    );
...
fn equalize_gpu_returns_3_passes() {
    ...
    assert_eq!(
        shaders.len(),
        3,
        "Equalize should use 3-pass Histogram256 reduction"
    );
    assert!(shaders[0].reduction_buffers[0].read_write);
    assert!(!shaders[2].reduction_buffers[0].read_write);
}
...
fn normalize_gpu_returns_3_passes() {
    ...
    assert_eq!(
        shaders.len(),
        3,
        "Normalize should use 3-pass Histogram256 reduction"
    );
    assert!(shaders[0].reduction_buffers[0].read_write);
    assert!(!shaders[2].reduction_buffers[0].read_write);
...
fn all_output_sizes_correct() {
    ...
    assert_eq!(
        Normalize::default().compute(&input, 16, 16).unwrap().len(),
        n
    );
    assert_eq!(
        FrequencyLow { sigma: 3.0 }
            .compute(&input, 16, 16)
            .unwrap()
            .len(),
        n
...
fn retinex_ssr_gpu_3_passes() {
    ...
    assert_eq!(
        passes.len(),
        3,
        "RetinexSSR: blur H + blur V + retinex apply"
    );
}
...
fn retinex_msr_gpu_has_multiple_passes() {
    ...
    assert!(
        passes.len() >= 10,
        "RetinexMSR should have many passes, got {}",
        passes.len()
    );
}
...
fn retinex_msrcr_gpu_has_multiple_passes() {
    ...
    assert!(
        passes.len() >= 10,
        "RetinexMSRCR should have many passes, got {}",
        passes.len()
    );
}
...
fn pyramid_detail_remap_gpu_multi_pass() {
    ...
    assert!(
        passes.len() >= 8,
        "Pyramid: should have downsample + remap passes, got {}",
        passes.len()
    );
}
