...
pub fn load_reference_vectors(path: &Path) -> Option<Vec<RefVector>> {
    ...
    let header: BinHeader = unsafe { std::ptr::read_unaligned(data.as_ptr() as *const BinHeader) };

    if &header.magic != b"ACE2" {
        return None;
    }
    ...
    for i in 0..count {
        let offset = 16 + i * 24;
        let v: RefVector =
            unsafe { std::ptr::read_unaligned(data[offset..].as_ptr() as *const RefVector) };
        vectors.push(v);
    }
